...
* In alignment with [](#section2), cursor values are URL-Safe strings that are opaque to clients.  Server providers should obfuscate cursors
values to prevent clients from interpreting cursors or forging new cursors.  Service providers should be able to easily detect forged cursor values and immediately return an `invalidCursor` as described in [](#section2.1)
...
This specification requests IANA to amends the SCIM Server-Related Schema URIs registry established by [RFC7643].
